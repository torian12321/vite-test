...
export type DefaultValues = { [fieldName: FieldName]: FieldValue };
...
export interface FormRendererProps {
  ...
  sxFieldsByType?: { [key in FIELD_TYPE]?: SxProps<Theme> };
  /** Styles to apply to fields based on field name. It will apply on top of sxFieldsByType */
  sxFieldsByName?: { [fieldName: FieldName]: SxProps<Theme> };
...
type DateDefaultValue = 'today' | ((string | Date) & { date?: string | Date });
type DateFieldProps = {
...
export type OnChange = (props: { values: FieldValuesObj }) => void;
